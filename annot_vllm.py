            ...
            for index, data in tqdm(enumerate(interactions)):
                if ('Personalized' in CONTAIN_SHOT) and (data['movie_title'] == random_movie):
                    ...
                    if 'Type' in CONTAIN_SHOT:
                        ...
                        present_case = '\n'.join(str(value)
                                                 for value in present_case.values())
                    elif CONTAIN_SHOT == 'None':
                        present_case = ''
                    cases = {'type': CONTAIN_SHOT, 'prompt': present_case}

                    llm_prompt = prompt_evaluate_in_likert_personalize_few_shot(
                        data, user_profile, cases)
                    llm_prompts.append(llm_prompt)
            print(len(interactions), len(llm_prompts))
            llm_answers, llm_results = call_local_llm_for_evaluate(
                llm_prompts, model, MODEL_NAME, mode="multi", temperature=float(TEMPEARTURE))

            for index, data in tqdm(enumerate(interactions)):
                llm_values['persuasiveness'][index] = (
                    float(llm_results[index][0]) if llm_results[index] is not None else None)
                llm_values['transparency'][index] = (
                    float(llm_results[index][1]) if llm_results[index] is not None else None)
                llm_values['accuracy'][index] = (
                    float(llm_results[index][2]) if llm_results[index] is not None else None)
                llm_values['satisfactory'][index] = (
                    float(llm_results[index][3]) if llm_results[index] is not None else None)
