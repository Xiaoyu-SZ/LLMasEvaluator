...
CONTAIN_SHOT = 'None' # All or Type or None # 在prompt中包含一组例子；还是与将标注的文本相符的一个例子；还是不包括
PERSONALIZED = '_personalized' if CONTAIN_USER_PROFILE else ''
TEMPEARTURE = 0.001
FILE_NAME = f'output/{MODEL_NAME}_T{str(TEMPEARTURE)}_{CONTAIN_SHOT}{PERSONALIZED}_correlation_results_split_all.csv' # 保存标注结果的路径
LOG_FILE_NAME = f'output/{MODEL_NAME}_T{str(TEMPEARTURE)}_{CONTAIN_SHOT}{PERSONALIZED}_user_split_log_all.txt' # 保存log结果的路径（目前是和LLM的交互）
...
def evaluate_llm(result_dict,user_dict):
    ...
    with open(FILE_NAME, 'w') as f:
        ...
        for result in tqdm(result_list):# 先只看前10个user的结果
            ...
            llm_answers,llm_results = call_local_llm_for_evaluate(llm_prompts,model,MODEL_NAME,mode="single",temperature=float(TEMPEARTURE))
                    # logger.write(llm_answer+'\n\n')
            for index,data in tqdm(enumerate(interactions)):
                for j,k in tqdm(enumerate(user_values.keys())):
                    llm_values[k][index]=(float(llm_results[index*4+j]) if llm_results[index*4+j] is not None else None)
                    
                    # llm_values['persuasiveness'].append(float(llm_result[0]))
                    # llm_values['transparency'].append(float(llm_result[1]))
                    # llm_values['accuracy'].append(float(llm_result[2]))
                    # llm_values['satisfactory'].append(float(llm_result[3]))
